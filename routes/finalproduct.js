...
router.post("/checkfinalproduct", function (req, res) {
  ...
  var q =
    "select * from finalproduct where productid=? and colorid=? and size=?";
  pool.query(
    q,
    [req.body.productid, req.body.colorid, req.body.size],
    function (error, result) {
      if (error) {
        res.status(500).json([]);
      } else {
        if (result.length == 1) res.status(200).json({ result: false });
...
router.post("/fetchallfinalproductsbyproductid", function (req, res, next) {
  q =
    "select fp.*,(select productname from product where productid=fp.productid)as productname,(select colorname from color where colorid=fp.colorid )as colorname from finalproduct as fp where productid=?";
  console.log(req.body);
...
router.post("/fetchsizebyproductid", function (req, res, next) {
  pool.query(
    "select * from finalproduct where productid=? group by size",
    [req.body.productid],
    function (error, result) {
      if (error) {
        res.status(500).json([]);
...
router.get("/fetchallfinalproducts", function (req, res, next) {
  q =
    "select fp.*,(select productname from product where productid=fp.productid)as productname,(select colorname from color where colorid=fp.colorid )as colorname from finalproduct as fp";
...
router.post(
  ...
  function (req, res, next) {
    ...
    pool.query(
      "insert into finalproduct (productid, colorid, size, price, offertype, offerprice, description, stock, productpicture) values(?,?,?,?,?,?,?,?,?)",
      [
        req.body.productid,
        req.body.colorid,
        req.body.size,
...
router.post("/updatefinalproductdata", function (req, res, next) {
  console.log("BODY:", req.body);
  pool.query(
    "update finalproduct set productid=?, colorid=?, size=?, price=?, offertype=?, offerprice=?, description=?, stock=? where finalproductid=?",
    [
      req.body.productid,
      req.body.colorid,
      req.body.size,
...
router.post(
  ...
  function (req, res, next) {
    pool.query(
      "update finalproduct set productpicture=? where finalproductid=?",
      [req.body.myfilename, req.body.finalproductid],
      function (error, result) {
        if (error) {
          console.log(error);
          res.status(500).json(false);
...
router.post("/deletefinalproduct", function (req, res, next) {
  pool.query(
    "delete from finalproduct where finalproductid=?",
    [req.body.finalproductid],
    function (error, result) {
      if (error) {
        res.status(500).json(false);
